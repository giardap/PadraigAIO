import { ModalRoot, ModalHeader, ModalContent, ModalFooter } from "@utils/modal";
import { Button, Text, React } from "@webpack/common";

// Padraig Branding Variables
const PADRAIG_LOGO_BASE64 = "data:image/png;base64,"
const BRAND_COLORS = {
    primary: "#F2F0E8",       // Beige/Champagne base
    primaryDark: "#E8E5DD",   // Darker beige
    accent1: "#E53E3E",       // Red accent
    accent2: "#FBB040",       // Yellow/Orange accent  
    accent3: "#38B2AC",       // Teal accent
    charcoal: "#4A5568",      // Charcoal grey
    success: "#38B2AC",       // Teal for success states
    warning: "#FBB040",       // Orange for warning states
    danger: "#E53E3E",        // Red for danger states
    text: "#2D3748",          // Dark text
    textMuted: "#718096"      // Muted text
};

// Global trading settings storage with new quick action settings
let globalTradingSettings = {
    slippage: "10",
    priorityFee: "0.00005",
    pool: "auto",
    skipPreflight: true,
    jitoOnly: false,
    // NEW: Quick buy SOL amounts
    quickBuyAmounts: ["0.01", "0.05", "0.1", "0.5", "1.0", "2.0"],
    // NEW: Quick sell percentages
    quickSellPercentages: ["25", "50", "75", "100"],
    // NEW: Dev buy preset amount
    devBuyAmount: "0.1"
};

// Functions to get and set global trading settings
export const getGlobalTradingSettings = () => ({ ...globalTradingSettings });

export const setGlobalTradingSettings = (settings: Partial<typeof globalTradingSettings>) => {
    globalTradingSettings = { ...globalTradingSettings, ...settings };
    
    // Persist to localStorage
    try {
        localStorage.setItem('padraig-trading-settings', JSON.stringify(globalTradingSettings));
    } catch (error) {
        console.warn("[TradingSettings] Failed to persist settings:", error);
    }
};

// Load settings from localStorage on module initialization
try {
    const saved = localStorage.getItem('padraig-trading-settings');
    if (saved) {
        const parsed = JSON.parse(saved);
        globalTradingSettings = { ...globalTradingSettings, ...parsed };
    }
} catch (error) {
    console.warn("[TradingSettings] Failed to load saved settings:", error);
}

export function StandaloneTradingSettingsModal(props: any) {
    const [slippage, setSlippage] = React.useState(globalTradingSettings.slippage);
    const [priorityFee, setPriorityFee] = React.useState(globalTradingSettings.priorityFee);
    const [pool, setPool] = React.useState(globalTradingSettings.pool);
    const [skipPreflight, setSkipPreflight] = React.useState(globalTradingSettings.skipPreflight);
    const [jitoOnly, setJitoOnly] = React.useState(globalTradingSettings.jitoOnly);
    
    // NEW: Quick action settings
    const [quickBuyAmounts, setQuickBuyAmounts] = React.useState<string[]>(globalTradingSettings.quickBuyAmounts);
    const [quickSellPercentages, setQuickSellPercentages] = React.useState<string[]>(globalTradingSettings.quickSellPercentages);
    const [devBuyAmount, setDevBuyAmount] = React.useState(globalTradingSettings.devBuyAmount);
    
    // NEW: Editing states for quick actions
    const [editingBuyAmounts, setEditingBuyAmounts] = React.useState(false);
    const [editingSellPercentages, setEditingSellPercentages] = React.useState(false);
    const [newBuyAmount, setNewBuyAmount] = React.useState("");
    const [newSellPercentage, setNewSellPercentage] = React.useState("");

    React.useEffect(() => {
        const current = getGlobalTradingSettings();
        setSlippage(current.slippage);
        setPriorityFee(current.priorityFee);
        setPool(current.pool);
        setSkipPreflight(current.skipPreflight);
        setJitoOnly(current.jitoOnly);
        setQuickBuyAmounts(current.quickBuyAmounts);
        setQuickSellPercentages(current.quickSellPercentages);
        setDevBuyAmount(current.devBuyAmount);
    }, []);

    const handleSave = () => {
        setGlobalTradingSettings({
            slippage,
            priorityFee,
            pool,
            skipPreflight,
            jitoOnly,
            quickBuyAmounts,
            quickSellPercentages,
            devBuyAmount
        });
        
        props.onClose();
    };

    // NEW: Quick buy amount management
    const addQuickBuyAmount = () => {
        const amount = parseFloat(newBuyAmount);
        if (amount > 0 && amount <= 100 && !quickBuyAmounts.includes(newBuyAmount)) {
            setQuickBuyAmounts([...quickBuyAmounts, newBuyAmount].sort((a, b) => parseFloat(a) - parseFloat(b)));
            setNewBuyAmount("");
        }
    };

    const removeQuickBuyAmount = (amount: string) => {
        setQuickBuyAmounts(quickBuyAmounts.filter(a => a !== amount));
    };

    // NEW: Quick sell percentage management
    const addQuickSellPercentage = () => {
        const percentage = parseFloat(newSellPercentage);
        if (percentage > 0 && percentage <= 100 && !quickSellPercentages.includes(newSellPercentage)) {
            setQuickSellPercentages([...quickSellPercentages, newSellPercentage].sort((a, b) => parseFloat(a) - parseFloat(b)));
            setNewSellPercentage("");
        }
    };

    const removeQuickSellPercentage = (percentage: string) => {
        setQuickSellPercentages(quickSellPercentages.filter(p => p !== percentage));
    };

    // NEW: Preset quick action configurations
    const setConservativeQuickActions = () => {
        setQuickBuyAmounts(["0.01", "0.05", "0.1", "0.2"]);
        setQuickSellPercentages(["10", "25", "50", "75"]);
        setDevBuyAmount("0.05");
    };

    const setBalancedQuickActions = () => {
        setQuickBuyAmounts(["0.01", "0.05", "0.1", "0.5", "1.0"]);
        setQuickSellPercentages(["25", "50", "75", "100"]);
        setDevBuyAmount("0.1");
    };

    const setAggressiveQuickActions = () => {
        setQuickBuyAmounts(["0.1", "0.5", "1.0", "2.0", "5.0"]);
        setQuickSellPercentages(["50", "75", "100"]);
        setDevBuyAmount("0.2");
    };

    const inputStyle = {
        backgroundColor: "#44403C",
        border: `1px solid ${BRAND_COLORS.primaryDark}`,
        borderRadius: "6px",
        padding: "8px 10px",
        color: BRAND_COLORS.primary,
        fontSize: "13px",
        width: "100%"
    };

    const labelStyle = {
        color: BRAND_COLORS.primary,
        fontSize: "13px",
        fontWeight: "600",
        marginBottom: "6px",
        display: "block"
    };

    // Preset options
    const slippagePresets = ["0.1", "0.5", "1", "5", "10", "15", "25"];
    const priorityFeePresets = ["0.00001", "0.00005", "0.0001", "0.0005", "0.001", "0.002"];

    // Risk level calculation
    const getRiskLevel = () => {
        const slippageNum = parseFloat(slippage) || 0;
        const priorityFeeNum = parseFloat(priorityFee) || 0;
        
        if (skipPreflight && slippageNum > 15) return "High";
        if (slippageNum > 10 || priorityFeeNum > 0.001) return "Medium";
        return "Low";
    };

    const riskLevel = getRiskLevel();
    const riskColor = riskLevel === "High" ? BRAND_COLORS.danger : 
                     riskLevel === "Medium" ? BRAND_COLORS.warning : 
                     BRAND_COLORS.success;

    return (
        <ModalRoot {...props}>
            <ModalHeader>
                <div style={{ 
                    display: "flex", 
                    justifyContent: "center", 
                    alignItems: "center", 
                    width: "100%",
                    padding: "12px 0"
                }}>
                    {PADRAIG_LOGO_BASE64 ? (
                        <img 
                            src={PADRAIG_LOGO_BASE64} 
                            alt="Padraig Logo" 
                            style={{
                                height: "42px",
                                width: "auto"
                            }}
                        />
                    ) : (
                        <div style={{
                            color: BRAND_COLORS.text,
                            fontSize: "18px",
                            fontWeight: "600"
                        }}>
                            PadraigAIO
                        </div>
                    )}
                </div>
            </ModalHeader>

            <ModalContent style={{ padding: "20px", maxHeight: "70vh", overflowY: "auto" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                    
                    {/* Risk Assessment */}
                    <div style={{
                        backgroundColor: riskColor + "20",
                        padding: "12px",
                        borderRadius: "6px",
                        border: `1px solid ${riskColor}`,
                        display: "flex",
                        justifyContent: "space-between",
                        alignItems: "center"
                    }}>
                        <div>
                            <Text style={{ 
                                fontSize: "12px", 
                                fontWeight: "600", 
                                color: BRAND_COLORS.text,
                                marginBottom: "4px"
                            }}>
                                🎯 Risk Level: <span style={{ color: riskColor }}>{riskLevel}</span>
                            </Text>
                            <Text style={{ 
                                fontSize: "10px", 
                                color: BRAND_COLORS.textMuted 
                            }}>
                                Est. Gas: {((parseFloat(priorityFee) || 0) + 0.000005).toFixed(6)} SOL • Dev Buy: {devBuyAmount} SOL
                            </Text>
                        </div>
                        <div style={{
                            backgroundColor: riskColor,
                            color: "white",
                            padding: "4px 8px",
                            borderRadius: "4px",
                            fontSize: "10px",
                            fontWeight: "600"
                        }}>
                            {riskLevel.toUpperCase()}
                        </div>
                    </div>
                    
                    {/* Slippage Settings */}
                    <div>
                        <label style={labelStyle}>Slippage Tolerance (%)</label>
                        <div style={{ marginBottom: "8px" }}>
                            <input 
                                type="number"
                                value={slippage} 
                                onChange={(e) => setSlippage(e.target.value)}
                                placeholder="10"
                                min="0.1"
                                max="50"
                                step="0.1"
                                style={inputStyle}
                            />
                        </div>
                        <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: "8px" }}>
                            {slippagePresets.map(preset => (
                                <button
                                    key={preset}
                                    onClick={() => setSlippage(preset)}
                                    style={{
                                        backgroundColor: slippage === preset ? BRAND_COLORS.accent3 : BRAND_COLORS.charcoal,
                                        color: "white",
                                        border: "none",
                                        borderRadius: "4px",
                                        padding: "4px 8px",
                                        fontSize: "11px",
                                        cursor: "pointer",
                                        fontWeight: "600"
                                    }}
                                >
                                    {preset}%
                                </button>
                            ))}
                        </div>
                        <Text style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted 
                        }}>
                            Higher slippage = more likely to execute, but worse price
                        </Text>
                    </div>

                    {/* Priority Fee Settings */}
                    <div>
                        <label style={labelStyle}>Priority Fee (SOL)</label>
                        <div style={{ marginBottom: "8px" }}>
                            <input 
                                type="number"
                                value={priorityFee} 
                                onChange={(e) => setPriorityFee(e.target.value)}
                                placeholder="0.00005"
                                min="0"
                                step="0.00001"
                                style={inputStyle}
                            />
                        </div>
                        <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: "8px" }}>
                            {priorityFeePresets.map(preset => (
                                <button
                                    key={preset}
                                    onClick={() => setPriorityFee(preset)}
                                    style={{
                                        backgroundColor: priorityFee === preset ? BRAND_COLORS.accent3 : BRAND_COLORS.charcoal,
                                        color: "white",
                                        border: "none",
                                        borderRadius: "4px",
                                        padding: "4px 8px",
                                        fontSize: "11px",
                                        cursor: "pointer",
                                        fontWeight: "600"
                                    }}
                                >
                                    {preset}
                                </button>
                            ))}
                        </div>
                        <Text style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted 
                        }}>
                            Higher fees = faster confirmation
                        </Text>
                    </div>

                    {/* Pool Selection */}
                    <div>
                        <label style={labelStyle}>Exchange Pool</label>
                        <select
                            value={pool}
                            onChange={(e) => setPool(e.target.value)}
                            style={inputStyle}
                        >
                            <option value="auto">Auto (Best Price)</option>
                            <option value="pump">Pump.fun</option>
                            <option value="raydium">Raydium</option>
                            <option value="pump-amm">Pump AMM</option>
                            <option value="launchlab">LaunchLab</option>
                            <option value="raydium-cpmm">Raydium CPMM</option>
                            <option value="bonk">Bonk</option>
                        </select>
                        <Text style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted, 
                            marginTop: "4px" 
                        }}>
                            Auto selects the best available pool for your trade
                        </Text>
                    </div>

                    {/* NEW: Dev Buy Amount Setting */}
                    <div>
                        <label style={labelStyle}>🎯 Dev Buy Amount (SOL)</label>
                        <div style={{ marginBottom: "8px" }}>
                            <input 
                                type="number"
                                value={devBuyAmount} 
                                onChange={(e) => setDevBuyAmount(e.target.value)}
                                placeholder="0.1"
                                min="0.01"
                                max="10"
                                step="0.01"
                                style={inputStyle}
                            />
                        </div>
                        <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: "8px" }}>
                            {["0.01", "0.05", "0.1", "0.2", "0.5", "1.0"].map(preset => (
                                <button
                                    key={preset}
                                    onClick={() => setDevBuyAmount(preset)}
                                    style={{
                                        backgroundColor: devBuyAmount === preset ? BRAND_COLORS.accent2 : BRAND_COLORS.charcoal,
                                        color: "white",
                                        border: "none",
                                        borderRadius: "4px",
                                        padding: "4px 8px",
                                        fontSize: "11px",
                                        cursor: "pointer",
                                        fontWeight: "600"
                                    }}
                                >
                                    {preset} SOL
                                </button>
                            ))}
                        </div>
                        <Text style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted 
                        }}>
                            Default SOL amount for buying your own tokens when creating
                        </Text>
                    </div>

                    {/* NEW: Quick Buy Amounts Configuration */}
                    <div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                            <label style={labelStyle}>⚡ Quick Buy Amounts (SOL)</label>
                            <button
                                onClick={() => setEditingBuyAmounts(!editingBuyAmounts)}
                                style={{
                                    backgroundColor: BRAND_COLORS.accent3,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "4px",
                                    padding: "4px 8px",
                                    fontSize: "10px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                {editingBuyAmounts ? "Done" : "Edit"}
                            </button>
                        </div>
                        
                        <div style={{ 
                            backgroundColor: BRAND_COLORS.primaryDark,
                            padding: "12px",
                            borderRadius: "6px",
                            border: `1px solid ${BRAND_COLORS.accent3}40`
                        }}>
                            <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: editingBuyAmounts ? "8px" : "0" }}>
                                {quickBuyAmounts.map(amount => (
                                    <div key={amount} style={{ display: "flex", alignItems: "center", gap: "2px" }}>
                                        <span style={{
                                            backgroundColor: BRAND_COLORS.accent3,
                                            color: "white",
                                            padding: "4px 8px",
                                            borderRadius: "4px",
                                            fontSize: "11px",
                                            fontWeight: "600"
                                        }}>
                                            {amount} SOL
                                        </span>
                                        {editingBuyAmounts && (
                                            <button
                                                onClick={() => removeQuickBuyAmount(amount)}
                                                style={{
                                                    backgroundColor: BRAND_COLORS.danger,
                                                    color: "white",
                                                    border: "none",
                                                    borderRadius: "50%",
                                                    width: "16px",
                                                    height: "16px",
                                                    fontSize: "10px",
                                                    cursor: "pointer",
                                                    display: "flex",
                                                    alignItems: "center",
                                                    justifyContent: "center"
                                                }}
                                            >
                                                ×
                                            </button>
                                        )}
                                    </div>
                                ))}
                            </div>
                            
                            {editingBuyAmounts && (
                                <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
                                    <input
                                        type="number"
                                        value={newBuyAmount}
                                        onChange={(e) => setNewBuyAmount(e.target.value)}
                                        placeholder="0.1"
                                        min="0.01"
                                        max="100"
                                        step="0.01"
                                        style={{
                                            ...inputStyle,
                                            width: "80px",
                                            fontSize: "11px"
                                        }}
                                    />
                                    <button
                                        onClick={addQuickBuyAmount}
                                        disabled={!newBuyAmount || parseFloat(newBuyAmount) <= 0}
                                        style={{
                                            backgroundColor: newBuyAmount && parseFloat(newBuyAmount) > 0 ? BRAND_COLORS.success : BRAND_COLORS.charcoal,
                                            color: "white",
                                            border: "none",
                                            borderRadius: "4px",
                                            padding: "4px 8px",
                                            fontSize: "10px",
                                            cursor: newBuyAmount && parseFloat(newBuyAmount) > 0 ? "pointer" : "not-allowed",
                                            fontWeight: "600"
                                        }}
                                    >
                                        Add
                                    </button>
                                </div>
                            )}
                        </div>
                        <Text style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted, 
                            marginTop: "4px" 
                        }}>
                            Quick buttons for buying tokens with these SOL amounts
                        </Text>
                    </div>

                    {/* NEW: Quick Sell Percentages Configuration */}
                    <div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                            <label style={labelStyle}>⚡ Quick Sell Percentages (%)</label>
                            <button
                                onClick={() => setEditingSellPercentages(!editingSellPercentages)}
                                style={{
                                    backgroundColor: BRAND_COLORS.accent1,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "4px",
                                    padding: "4px 8px",
                                    fontSize: "10px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                {editingSellPercentages ? "Done" : "Edit"}
                            </button>
                        </div>
                        
                        <div style={{ 
                            backgroundColor: BRAND_COLORS.primaryDark,
                            padding: "12px",
                            borderRadius: "6px",
                            border: `1px solid ${BRAND_COLORS.accent1}40`
                        }}>
                            <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: editingSellPercentages ? "8px" : "0" }}>
                                {quickSellPercentages.map(percentage => (
                                    <div key={percentage} style={{ display: "flex", alignItems: "center", gap: "2px" }}>
                                        <span style={{
                                            backgroundColor: BRAND_COLORS.accent1,
                                            color: "white",
                                            padding: "4px 8px",
                                            borderRadius: "4px",
                                            fontSize: "11px",
                                            fontWeight: "600"
                                        }}>
                                            {percentage}%
                                        </span>
                                        {editingSellPercentages && (
                                            <button
                                                onClick={() => removeQuickSellPercentage(percentage)}
                                                style={{
                                                    backgroundColor: BRAND_COLORS.danger,
                                                    color: "white",
                                                    border: "none",
                                                    borderRadius: "50%",
                                                    width: "16px",
                                                    height: "16px",
                                                    fontSize: "10px",
                                                    cursor: "pointer",
                                                    display: "flex",
                                                    alignItems: "center",
                                                    justifyContent: "center"
                                                }}
                                            >
                                                ×
                                            </button>
                                        )}
                                    </div>
                                ))}
                            </div>
                            
                            {editingSellPercentages && (
                                <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
                                    <input
                                        type="number"
                                        value={newSellPercentage}
                                        onChange={(e) => setNewSellPercentage(e.target.value)}
                                        placeholder="25"
                                        min="1"
                                        max="100"
                                        step="1"
                                        style={{
                                            ...inputStyle,
                                            width: "80px",
                                            fontSize: "11px"
                                        }}
                                    />
                                    <button
                                        onClick={addQuickSellPercentage}
                                        disabled={!newSellPercentage || parseFloat(newSellPercentage) <= 0}
                                        style={{
                                            backgroundColor: newSellPercentage && parseFloat(newSellPercentage) > 0 ? BRAND_COLORS.success : BRAND_COLORS.charcoal,
                                            color: "white",
                                            border: "none",
                                            borderRadius: "4px",
                                            padding: "4px 8px",
                                            fontSize: "10px",
                                            cursor: newSellPercentage && parseFloat(newSellPercentage) > 0 ? "pointer" : "not-allowed",
                                            fontWeight: "600"
                                        }}
                                    >
                                        Add
                                    </button>
                                </div>
                            )}
                        </div>
                        <Text style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted, 
                            marginTop: "4px" 
                        }}>
                            Quick buttons for selling these percentages of token holdings
                        </Text>
                    </div>

                    {/* Advanced Options */}
                    <div>
                        <label style={labelStyle}>Advanced Options</label>
                        <div style={{ 
                            backgroundColor: BRAND_COLORS.primaryDark,
                            padding: "12px",
                            borderRadius: "6px",
                            border: `1px solid ${BRAND_COLORS.accent2}40`
                        }}>
                            <label style={{ 
                                display: "flex", 
                                alignItems: "center", 
                                gap: "8px", 
                                fontSize: "12px",
                                color: BRAND_COLORS.text,
                                cursor: "pointer",
                                marginBottom: "8px"
                            }}>
                                <input 
                                    type="checkbox"
                                    checked={skipPreflight}
                                    onChange={(e) => setSkipPreflight(e.target.checked)}
                                    style={{ margin: 0 }}
                                />
                                <span>Skip Preflight Checks</span>
                            </label>
                            <Text style={{ 
                                fontSize: "10px", 
                                color: BRAND_COLORS.textMuted, 
                                marginLeft: "24px",
                                marginBottom: "8px"
                            }}>
                                Skips transaction simulation for faster execution (higher risk)
                            </Text>
                            
                            <label style={{ 
                                display: "flex", 
                                alignItems: "center", 
                                gap: "8px", 
                                fontSize: "12px",
                                color: BRAND_COLORS.text,
                                cursor: "pointer"
                            }}>
                                <input 
                                    type="checkbox"
                                    checked={jitoOnly}
                                    onChange={(e) => setJitoOnly(e.target.checked)}
                                    style={{ margin: 0 }}
                                />
                                <span>Jito Bundle Only</span>
                            </label>
                            <Text style={{ 
                                fontSize: "10px", 
                                color: BRAND_COLORS.textMuted, 
                                marginLeft: "24px"
                            }}>
                                Routes transaction only through Jito for MEV protection
                            </Text>
                        </div>
                    </div>

                    {/* NEW: Quick Action Presets */}
                    <div>
                        <label style={labelStyle}>🎯 Quick Action Presets</label>
                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px" }}>
                            <button
                                onClick={setConservativeQuickActions}
                                style={{
                                    backgroundColor: BRAND_COLORS.success,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
                                    padding: "8px 12px",
                                    fontSize: "11px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                🐌 Conservative
                            </button>
                            <button
                                onClick={setBalancedQuickActions}
                                style={{
                                    backgroundColor: BRAND_COLORS.warning,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
                                    padding: "8px 12px",
                                    fontSize: "11px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                ⚡ Balanced
                            </button>
                            <button
                                onClick={setAggressiveQuickActions}
                                style={{
                                    backgroundColor: BRAND_COLORS.danger,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
                                    padding: "8px 12px",
                                    fontSize: "11px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                🚀 Aggressive
                            </button>
                        </div>
                        <div style={{ 
                            fontSize: "10px", 
                            color: BRAND_COLORS.textMuted,
                            display: "grid",
                            gridTemplateColumns: "1fr 1fr 1fr",
                            gap: "4px"
                        }}>
                            <div>Conservative: Small amounts, partial sells</div>
                            <div>Balanced: Mixed amounts, standard sells</div>
                            <div>Aggressive: Large amounts, full sells</div>
                        </div>
                    </div>

                    {/* Quick Presets for Core Settings */}
                    <div>
                        <label style={labelStyle}>⚡ Core Setting Presets</label>
                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                            <button
                                onClick={() => {
                                    setSlippage("1");
                                    setPriorityFee("0.00001");
                                    setPool("auto");
                                    setSkipPreflight(false);
                                    setJitoOnly(false);
                                }}
                                style={{
                                    backgroundColor: BRAND_COLORS.success,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
                                    padding: "8px 12px",
                                    fontSize: "11px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                🐌 Conservative Core
                            </button>
                            <button
                                onClick={() => {
                                    setSlippage("10");
                                    setPriorityFee("0.0005");
                                    setPool("auto");
                                    setSkipPreflight(false);
                                    setJitoOnly(false);
                                }}
                                style={{
                                    backgroundColor: BRAND_COLORS.warning,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
                                    padding: "8px 12px",
                                    fontSize: "11px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                ⚡ Balanced Core
                            </button>
                            <button
                                onClick={() => {
                                    setSlippage("25");
                                    setPriorityFee("0.002");
                                    setPool("auto");
                                    setSkipPreflight(true);
                                    setJitoOnly(false);
                                }}
                                style={{
                                    backgroundColor: BRAND_COLORS.danger,
                                    color: "white",
                                    border: "none",
                                    borderRadius: "6px",
                                    padding: "8px 12px",
                                    fontSize: "11px",
                                    cursor: "pointer",
                                    fontWeight: "600"
                                }}
                            >
                                🚀 Aggressive Core
                            </button>
                        </div>
                    </div>

                    {/* Current Settings Summary */}
                    <div style={{
                        backgroundColor: BRAND_COLORS.accent3 + "20",
                        padding: "12px",
                        borderRadius: "6px",
                        border: `1px solid ${BRAND_COLORS.accent3}`
                    }}>
                        <Text style={{ 
                            fontSize: "12px", 
                            fontWeight: "600", 
                            color: BRAND_COLORS.text,
                            marginBottom: "8px"
                        }}>
                            📊 Current Settings Summary
                        </Text>
                        <div style={{ 
                            fontSize: "11px", 
                            color: BRAND_COLORS.textMuted,
                            fontFamily: "monospace"
                        }}>
                            Slippage: {slippage}% • Priority Fee: {priorityFee} SOL • Pool: {pool}<br/>
                            Skip Preflight: {skipPreflight ? 'Yes' : 'No'} • Jito Only: {jitoOnly ? 'Yes' : 'No'}<br/>
                            Dev Buy: {devBuyAmount} SOL • Quick Buy: [{quickBuyAmounts.join(', ')}] SOL<br/>
                            Quick Sell: [{quickSellPercentages.join(', ')}]%
                        </div>
                    </div>
                </div>
            </ModalContent>

            <ModalFooter>
                <button
                    onClick={handleSave}
                    style={{
                        backgroundColor: BRAND_COLORS.success,
                        color: "#fff",
                        fontWeight: "bold",
                        fontSize: "16px",
                        padding: "12px 24px",
                        borderRadius: "6px",
                        border: "none",
                        cursor: "pointer",
                        transition: "all 0.2s ease",
                        marginRight: "12px"
                    }}
                >
                    ✅ Save Settings
                </button>
                <Button
                    color={Button.Colors.TRANSPARENT}
                    look={Button.Looks.LINK}
                    onClick={props.onClose}
                >
                    Cancel
                </Button>
            </ModalFooter>
        </ModalRoot>
    );
}